...
func NewAddService(
	indexService *IndexService,
	updateIndexService *UpdateIndexService,
	pathResolver *util.PathResolver) *AddService {
	return &AddService{
		indexService:       indexService,
		updateIndexService: updateIndexService,
...
func collectPaths(index *domain.Index, resolvedPaths []util.ResolvedPath) ([]string, []string, error) {
	...
	for _, resolved := range resolvedPaths {
		...
		case util.File, util.NonExistent:
